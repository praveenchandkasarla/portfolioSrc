...
export class Ng2parallax implements OnInit {
  ...
  ngOnInit() {
    ...
    function execute() {
      let scrollTop = (window.pageYOffset !== undefined) ? window.pageYOffset : window(document.documentElement || document.body.parentNode || document.body).scrollTop;
      let speed = (scrollTop / _speed);
      if (isMobile) {
        speed = speed * .10;
      }
      if (speed === 0) {
